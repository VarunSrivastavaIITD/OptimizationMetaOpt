...
class iMAML(GBML):
    def __init__(self, args):
        super().__init__(args)
        self._init_net()
        print(f'Running iMAML with freeze pre-final layers = {args.freeze}')
        self._init_opt(args.freeze)
        self.lamb = 100.0
        self.n_cg = 1
        return None
    ...
    def hv_prod(self, in_grad, x, params):
        hv = torch.autograd.grad(in_grad,
                                 params,
                                 retain_graph=True,
                                 grad_outputs=x)
        hv = tuple(h.contiguous() for h in hv)
    ...
    def outer_loop(self, batch, is_train):
        ...
        for (train_input, train_target, test_input,
            ...
            with higher.innerloop_ctx(self.network,
                                      self.inner_optimizer, track_higher_grads=False) as (fmodel,
                                                                    diffopt):
